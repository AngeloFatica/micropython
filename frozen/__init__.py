...
class PSNERGY_Manometer(Screen):
    ...
    def send_PR(self, MA: float, MA2: float):
        self.write_global_text("manometer", "tMA", '{:.2f}'.format(MA) )
        self.write_global_text("manometer", "tMA2", '{:.2f}'.format(MA2) )
        MA = 66/self.grPRScale*MA + 66
        MA2 = 66/self.grPRScale*MA2 + 66
        self.send_to_graph(self.gPR, 0, int(MA))
        self.send_to_graph(self.gPR, 1, int(MA2))
